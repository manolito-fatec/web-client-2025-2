...
import type { ChartDataStructure, Forecaster, MonthTicket } from '@/types/InsightType/Insight.ts'
...
function calculateChartData(data: Forecaster[]) {
  ...
  labels.value = [...new Set(data.map((r) => formatMonth(r.futureDate)))].sort((a, b) => {
    const [ma, ya] = a.split('/')
    const [mb, yb] = b.split('/')
...
function sumTicketsByMonth(forecasters: Forecaster[]) {
  const monthlyTotals = forecasters.reduce(
    (acc, curr) => {
...
<template>
  <span>
    ...
    <p>
      Com base na previsão, o mês de maior volume geral de chamados será
      <b> {{ convertLabelToPtBr(sumMonthTickets[0]?.month) }}</b>
    </p>
